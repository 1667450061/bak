...
#define DEBUG_DBG	0x008
#define DEBUG_INFO	0x004
#define DEBUG_ERR	0x002

static enum lcusb_xceiv_events plugin_stat;
/* debug log control */
static uint32_t g_debug_lvl = DEBUG_INFO + DEBUG_ERR ;

/* debug log control */
//#define DBG(fmt, arg...)	printk(KERN_DEBUG "u2d:D: " fmt , ## arg)
#define DBG(fmt, arg...)	do { \
							if(g_debug_lvl & DEBUG_DBG) \
							printk(KERN_DEBUG"u2d:D: " fmt , ## arg); \
							}while(0)

#define INFO(fmt, arg...)	do { \
							if(g_debug_lvl & DEBUG_INFO) \
							printk(KERN_DEBUG"u2d:I: " fmt , ## arg); \
							}while(0)

#define ERR(fmt, arg...)	do { \
							if(g_debug_lvl & DEBUG_ERR) \
							printk(KERN_ERR"u2d:E: " fmt , ## arg); \
							}while(0)
...
/* hw wait count */
#define MAX_WAIT_COUNT			(350 * 1000)
...
+-------------------------------------------------------------------------*/
static unsigned long charge_mode = 0;
static DEFINE_SPINLOCK(mod_splk);

static ssize_t comip_charge_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned long flags;
	unsigned int ret;

	spin_lock_irqsave(&mod_splk, flags);
	ret = sprintf(buf, "%ld\n", charge_mode);
	spin_unlock_irqrestore(&mod_splk, flags);

	return ret;
}

static ssize_t comip_charge_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long flags;
	int ret;

	spin_lock_irqsave(&mod_splk, flags);
	ret = kstrtoul(buf, 10, &charge_mode);
	spin_unlock_irqrestore(&mod_splk, flags);
...
static DEVICE_ATTR(charge_mode, S_IRUGO | S_IWUSR, comip_charge_mode_show, comip_charge_mode_store);

static ssize_t dbg_level_show(struct device *dev, struct device_attribute *drv, char *buf)
{
    return sprintf(buf, "0x%0x\n", g_debug_lvl);
}

static ssize_t dbg_level_store(struct device *dev, struct device_attribute *drv, const char *buf,
    size_t count)
{
    g_debug_lvl = simple_strtoul(buf, NULL, 16);
    return count;
}

static DEVICE_ATTR(debuglevel, S_IRUGO | S_IWUSR, dbg_level_show, dbg_level_store);
...
static void usb_enum_done(void)
{
	...
	case 0:
		...
		printk("dev->gadget.speed = USB_SPEED_HIGH\n");
		break;
	case 1:
		dev->gadget.speed = USB_SPEED_FULL;
		printk("dev->gadget.speed = USB_SPEED_FULL\n");
		break;
	case 2:
		dev->gadget.speed = USB_SPEED_LOW;
		printk("dev->gadget.speed = USB_SPEED_LOW\n");
		break;
	default:
		return;
...
int comip_usb_power_set(int onoff)
{
	...
	if ( ((onoff == 1) && (state_count == 0))
		||((onoff == 0) && (state_count == 1)) ) {
		ret = pmic_voltage_set(PMIC_POWER_USB, 0,
					onoff ? PMIC_POWER_VOLTAGE_ENABLE : PMIC_POWER_VOLTAGE_DISABLE);

		ret |= pmic_voltage_set(PMIC_POWER_USB, 1,
					onoff ? PMIC_POWER_VOLTAGE_ENABLE : PMIC_POWER_VOLTAGE_DISABLE);
		mdelay(10);
	}
	...
	if((state_count > 2)||(state_count < 0)) {
		pr_err("%s: E on=%d, count=%d\n", __func__, onoff, state_count);
		state_count = ((state_count > 2)?2:state_count);
		state_count = ((state_count < 0)?0:state_count);
	} else {
		pr_info("%s: on=%d, count=%d\n", __func__, onoff, state_count);
	}

    usb_power_state = state_count ? 1 : 0;
...
//#if 1
	...
	/*set USB PHY reset */
	val = 0x01;
	comip_u2d_write_reg32(val, CTL_POR_OTGPHY_CTRL);

	u2d_clk_set(0);

	if (cpu_is_lc1860_eco2()) {
		/* usb phy param */
		val = comip_u2d_read_reg32(CTL_OTGPHY_PARAM_OVERRIDE);
		val &= ~(0x3<<22);
		val |= (1<<22);
		comip_u2d_write_reg32(val, CTL_OTGPHY_PARAM_OVERRIDE);
	}
	...
	comip_u2d_write_reg32(0x1, CTL_OTG_CORE_RST_CTRL);
	mdelay(1);

	/*set USB0ID detection*/
	val = comip_u2d_read_reg32(CTL_OTGPHY_CHARGE_CTRL);
	val |= (1<<CTL_OTGPHY_CHARGE_CTRL_ACAENB0_OTGPHY);
	comip_u2d_write_reg32(val, CTL_OTGPHY_CHARGE_CTRL);
...
static irqreturn_t comip_u2d_irq(int irq, void *dev_id)
{
	...
	usb_gintsts = (usb_gintsts & usb_gintmsk);
	...
	if (usb_gintsts & USB_GINTSTS_ENUMDONE) {//speed enum completed
		...
		if((dev->linkstat != LCUSB_EVENT_VBUS)&&(dev->vchgstat == 1)){
...
{
	struct comip_u2d *dev = &memory;
	struct comip_udc_platform_data *pdata = dev->dev->platform_data;
	int ret;
	int val;

	INFO("%s, ##is_on=0x%x\n", __func__, is_on);
	dev_info(dev->dev, "lll: pullup: %d\n", is_on);
	...
	if (!is_on) {
		...
		if(android_have_functions()&&(dev->insert_flag == 0)){
			INFO("%s, pullup = 1\n", __func__);
			u2d_reinit(dev);
			if( pdata->flag | USB_PD_FLAGS_USB0VBUS) {
				//USB0VBUS
				val = comip_u2d_read_reg32(CTL_OTGPHY_CTRL);
				val |= (3<<11);
				comip_u2d_write_reg32(val, CTL_OTGPHY_CTRL);
				mdelay(10);
			}
			ret = u2d_usb_init();
			if (ret != 0) {
...
int comip_u2d_charger_detect(struct comip_u2d *dev)
{
    ...
    do {
        val = comip_u2d_read_reg32(CTL_OTG_PHY_STATUS);
        if(val&0x03){
            dev_info(dev->dev,"otg_ohy_status detected = 0x%x\n",val);
            break;
        }
    ...
    if((chargertype == LCUSB_EVENT_VBUS)||(val == 0x01)||(val == 0x02)){
...
static void comip_charger_work(struct work_struct *work)
{
    ...
    if(dev->vchgstat){
        ...
        if(dev->plugin == 1){
            ...
            delaytime = jiffies + msecs_to_jiffies(300);
        }
...
static int __init comip_u2d_probe(struct platform_device *pdev)
{
	...
	dev->linkstat = 0;
	dev->vchgstat = 0;
	if(pmic_vchg_input_state_get() == 1){
		dev->plugin = 1;
	}else{
		dev->plugin = 0;
	}
	...
	device_create_file(dev->dev, &dev_attr_charge_mode);
	device_create_file(dev->dev, &dev_attr_debuglevel);
...
static int comip_u2d_remove(struct platform_device *pdev)
{
	...
	device_remove_file(dev->dev, &dev_attr_charge_mode);
	device_remove_file(dev->dev, &dev_attr_debuglevel);
